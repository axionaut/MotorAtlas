...
type Vehicle = { id: string; canonicalName: string; make: string; model: string; market: string; modelYear: number | null; trim: string | null; bodyStyle: string | null; powertrain: string | null; transmission: string | null; driveType: string | null; completeness: number; reviewStatus: string; observationCount: number; sourceCount: number; specScore: number | null; scoreCoverage: number; scoreCriteria: Array<{ key: string; label: string; value: number; normalised: number }> };
...
function SpecScoreCell({ vehicle }: { vehicle: Vehicle }) {
  if (vehicle.specScore === null) return <span className="font-mono text-xs text-black/35">No graded evidence</span>;
  return <div className="flex items-center gap-3"><Progress value={vehicle.specScore} className="bg-black/10 [&_[data-slot=progress-indicator]]:bg-[#16612d]" /><span className="w-20 font-mono text-xs"><b className="text-sm">{vehicle.specScore}</b> · {vehicle.scoreCoverage}%</span></div>;
}
...
function ComparePanel({ data, selectedNames, onOpenCatalog }: { data: CompareData; selectedNames: string[]; onOpenCatalog: () => void }) {
  ...
  const scoreRow = <TableRow className="bg-[#f7f6f2]"><TableCell className="font-semibold">Spec score</TableCell>{data.vehicles.map((vehicle) => <TableCell key={vehicle.id}>{vehicle.specScore === null ? <span className="text-sm text-black/40">No graded evidence</span> : <><b className="font-mono text-xl">{vehicle.specScore}</b><div className="mt-1 text-xs text-black/45">{vehicle.scoreCoverage}% of criteria evidenced</div></>}</TableCell>)}</TableRow>;
  ...
  return <section className="overflow-hidden rounded-2xl border border-black/10 bg-white"><div className="border-b border-black/10 p-5"><h2 className="text-lg font-bold">Evidence-aware comparison</h2><p className="text-sm text-black/50">Best-supported values lead; their source remains visible. The spec score weights safety, efficiency, emissions, power, range, torque and occupant protection, and only counts criteria with evidence.</p></div><Table><TableHeader><TableRow className="bg-[#171a1c] hover:bg-[#171a1c]"><TableHead className="w-44 text-white/55">Attribute</TableHead>{data.vehicles.map((vehicle) => <TableHead key={vehicle.id} className="min-w-[220px] whitespace-normal py-4 text-white"><span className="block text-base">{vehicle.canonicalName}</span><span className="mt-1 block font-mono text-xs font-normal text-[#f7b955]">{vehicle.market} · {vehicle.completeness}%</span></TableHead>)}</TableRow></TableHeader><TableBody>{scoreRow}{identityRows.map(([label, key]) => <TableRow key={key}><TableCell className="font-semibold text-black/55">{label}</TableCell>{data.vehicles.map((vehicle) => <TableCell key={vehicle.id}>{key === "completeness" ? `${vehicle[key] ?? 0}%` : String(vehicle[key] ?? "—")}</TableCell>)}</TableRow>)}{data.attributes.map((attribute) => <TableRow key={attribute.attributeKey}><TableCell className="font-semibold text-black/55">{pretty(attribute.attributeKey)}</TableCell>{attribute.values.map((value, index) => <TableCell key={index}><b>{shownValue(value)}</b>{value && <div className="mt-1 text-xs text-black/40">{value.sourceName} · {value.confidence}%</div>}</TableCell>)}</TableRow>)}</TableBody></Table>{!data.attributes.length && <div className="border-t p-5 text-sm text-black/45">These records have no technical observations yet. Open a vehicle and attach sourced evidence.</div>}</section>;
}
